...
class CasApplicationTests {
     ...
     * 测试步骤 1
     * <p>
     * 条件:服务配置正确的数据库连接,并手动新建数据库后使用 help/doc/sql/CAS 表结构.sql 建表
     * <p>
     * 初始化测试数据 (推荐使用本单元测试进行初始化测试数据,可以确认服务配置是否正常,如果使用 help/doc/sql/CAS 测试数据.sql 初始化测试数据,请跳过此单元测试直接执行测试步骤 2)
     * <p>
